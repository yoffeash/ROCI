...
summary(ICUdays)
cbind(IRR = exp(coef(ICUdays)), 
      exp(confint(ICUdays)))[2,]
...
imgs_MV <- imgs %>% filter(days_MV > 0)
mvdays_nb <- glm.nb(days_MV ~ Total+APACHE+Age+non_white+Male+Immsupp, data=imgs_MV)
summary(mvdays_nb)
cbind(IRR = exp(coef(mvdays_nb)), 
      exp(confint(mvdays_nb)))[2,]
```
